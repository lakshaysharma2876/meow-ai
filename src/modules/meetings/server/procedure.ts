...
import {z} from "zod";
import { DEFAULT_PAGE, DEFAULT_PAGE_SIZE, MAX_PAGE_SIZE, MIN_PAGE_SIZE, UNRESTRICTED_EMAILS } from "@/constants";
    ...
    .query(async ({input,ctx}) =>{
        ...
        const userSpeakers = await db
          ...
          .then((users) =>
            users.map((u) => ({
              ...
              image: 
                u.image??
                GeneratedAvatarUri( {seed: u.name, variant:"initials" }),
            }))
          );
        const agentSpeakers = await db
          ...
          .then((agents) =>
            agents.map((u) => ({
              ...u,
              image: GeneratedAvatarUri( {seed: u.name, variant:"initials" })
            }))
          );
        ...
        const transcriptWithSpeakers = transcript.map((item)=>{
            ...
            if(!speaker){
                return {
                  ...
                  user : {
                    name:"Unknown",
                    image: GeneratedAvatarUri( {seed: "Unknown", variant:"initials" })
                  },
                }
              };
